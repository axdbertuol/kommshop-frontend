...
function Sidebar({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
  ...
  const sizes = { min: 24, max: '48rem' }

  return (
    <Collapsible
      open={isOpen}
      onOpenChange={setIsOpen}
      className={cn('fixed z-10 top-0 flex', className)}
      {...props}
    >
      <NavigationMenu
        data-orientation="vertical"
        className={cn(
          'w-full pt-8 border border-r-2  ring ring-primary rounded md:h-screen bg-neutral-950 bg-opacity-30 items-start',
          isOpen ? `md:w-[12rem] md:min-w-[12rem]` : ''
        )}
      >
        <ul
          data-orientation="vertical"
          className="w-full mt-16 flex flex-col divide-y border-t border-b"
        >
